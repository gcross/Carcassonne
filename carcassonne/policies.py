    ...
    class BoundPolicy(BoundPolicyBase):
        ...
        def converged(self):
            ...
            if last is not None and current is not None:
                print("absolute difference is",norm(current.toArray()-last.toArray()),"relative difference is",norm(current.toArray()-last.toArray())/norm(current.toArray()+last.toArray())*2,"average norm is",norm(current.toArray()+last.toArray())/2,"threshold is",self.parent.threshold)
            return last is not None and current is not None and (norm(current.toArray()+last.toArray()) < 1e-15 or norm(current.toArray()-last.toArray())/norm(current.toArray()+last.toArray())*2 < self.parent.threshold)
        def reset(self):
            self.last = None
            self.current = None
        def update(self):
            self.last = self.current
            self.current = self.system.state_center_data.ravel()
# }}}
# }}}
