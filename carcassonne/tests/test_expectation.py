...
class TestExpectation(TestCase): # {{{
    ...
    def generateData():
        ...
        corners = tuple(SparseCorner(mapSparseChunkValues(DenseCorner,corner_tensor)) for corner_tensor in corners_tensor)
    ...
    def test_absorbCenter(self,i=irange(0,3)): # {{{
        ...
        sides = list(sides)
        corners[i] = corners[i].absorbFromLeft(i,sides[(i+1)%4])
        sides[i] = sides[i].absorbCenterSOS(i,state_center_data,operator_center_tensor)
        corners[(i-1)%4] = corners[(i-1)%4].absorbFromRight((i-1)%4,sides[(i-1)%4])
        self.assertSparseTensorWithWrappedDataAlmostEqual(expectation.corners[0].tensor,corners[0].tensor)
        self.assertSparseTensorWithWrappedDataAlmostEqual(expectation.corners[1].tensor,corners[1].tensor)
        self.assertSparseTensorWithWrappedDataAlmostEqual(expectation.corners[2].tensor,corners[2].tensor)
        self.assertSparseTensorWithWrappedDataAlmostEqual(expectation.corners[3].tensor,corners[3].tensor)
